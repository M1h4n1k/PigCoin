...
def make_bet(
    db: Session, user: models.User, decoration: models.Decoration, amount: int
) -> models.Decoration:
    ...
    decoration.betting_ends_at = max(
        decoration.betting_ends_at, datetime.now() + timedelta(hours=1)
    )
    db.commit()
    return decoration
...
def get_decoration_on_auction(db: Session) -> models.Decoration | None:
    return (
        db.query(models.Decoration)
        .filter(models.Decoration.betting_ends_at > datetime.now())
        .order_by(asc(models.Decoration.betting_ends_at))
        .first()
    )
