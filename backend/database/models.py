...
class User(Base):
    ...
    energy_last_used: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=func.now()
    )

    last_ad_collected: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=datetime(2021, 1, 1)
    )

    @property
    def can_collect_ad(self) -> bool:
        return (datetime.now() - self.last_ad_collected) >= timedelta(
            hours=1, minutes=12
        )  # roughly 20 times a day

    # no idea why I've created this column
    last_coin_collected: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=func.now()
    )
    ...
    def auto_coins(self) -> int:
        ...
        hours_passed = min((datetime.now() - self.energy_last_used).seconds / 3600, 5)
        if hours_passed < 0.3:
            return 0
    ...
    def current_energy(self) -> int:
        return min(
            ...
            + (datetime.now() - self.energy_last_used).seconds * self.refill_rate,
        )
    ...
    def position(self) -> int:
        qr = (
            object_session(self).query(User).filter(User.total_coins > self.total_coins)
        )
    ...
    def position_in_club(self) -> int:
        qr = (
            ...
            .query(User)
            .filter(
                and_(User.club_id == self.club_id, User.total_coins > self.total_coins)
            )
        )
        return qr.count() + 1

    _free_turbo: Mapped[int] = mapped_column(
        BIGINT, default=3, index=True
    )  # 3 free turbo per day
    free_turbo_last_used: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=func.now()
    )
    ...
    free_refills_last_used: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=func.now()
    )
    ...
    def refill_rate(self) -> int:
        refill_boosts = self.boosts.filter(
            UserBoost.boost_type == "refill_rate"
        ).first()
    ...
    referrals: Mapped[list["User"]] = relationship(
        "User",
        remote_side=[referrer_tg_id],
        order_by="User.total_coins.desc()",
        lazy="dynamic",
    )
    tasks_completed: Mapped[list["Task"]] = relationship("Task", secondary="user_tasks")
    decorations: Mapped[list["Decoration"]] = relationship(
        "Decoration", back_populates="owner", foreign_keys="Decoration.owner_tg_id"
    )
...
class UserBoost(Base):
    ...
    boost: Mapped["Boost"] = relationship("Boost", lazy="joined")
...
class Club(Base):
    ...
    def total_coins(self):
        ...
        total_coins = (
            db.query(func.sum(User.total_coins))
            .filter(User.club_id == self.id)
            .scalar()
        )
...
class Transaction(Base):
    ...
    from_user_id: Mapped[int] = mapped_column(
        BIGINT, ForeignKey("users.tg_id", ondelete="NO ACTION")
    )
...
class Decoration(Base):
    ...
    last_bet_user_id: Mapped[int] = mapped_column(
        BIGINT, ForeignKey("users.tg_id", ondelete="SET NULL"), nullable=True
    )
    type: Mapped[str] = mapped_column(VARCHAR(255))
    owner_tg_id: Mapped[int] = mapped_column(
        BIGINT, ForeignKey("users.tg_id", ondelete="SET NULL"), nullable=True
    )
    betting_ends_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), index=True
    )

    owner = relationship(
        "User", foreign_keys=[owner_tg_id], back_populates="decorations"
    )
