...
def test_make_transaction(mocker, dummy_user):
    ...
    get_user_patch = mocker.patch(
        "routers.user.crud.users.get_user_by_uid", Mock(return_value=dummy_user)
    )
    ...
    response = client_l.post(
        "/api/transactions", json={"to_user_uid": dummy_user.uid, "amount": 100}
    )
...
def test_make_transaction_no_coins(mocker, dummy_user):
    ...
    client_l = get_client_with_dep(patch_get_user)
    response = client_l.post(
        "/api/transactions", json={"to_user_uid": dummy_user.uid, "amount": 1000}
    )
...
def test_make_transaction_to_self(mocker):
    ...
    client_l = get_client_with_dep(patch_get_user)
    response = client_l.post(
        "/api/transactions", json={"to_user_uid": "123", "amount": 100}
    )
