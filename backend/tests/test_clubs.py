...
def test_create_join_club(mocker, dummy_user):
    ...
    patch_crud_club_get = mocker.patch(
        "routers.clubs.crud.clubs.get_club_by_name", Mock()
    )
...
async def test_create_club(mocker):
    patch_crud_club_get = mocker.patch(
        "routers.clubs.crud.clubs.get_club_by_name", Mock(return_value=None)
    )
    patch_crud_user = mocker.patch(
        "routers.clubs.crud.users.update_user_club", Mock(return_value=None)
    )
    patch_crud_club_create = mocker.patch(
        "routers.clubs.crud.clubs.create_club", Mock(return_value=Mock(id=1))
    )
    patch_bot_get_chat = mocker.patch(
        "routers.clubs.bot.get_chat", AsyncMock(return_value=Mock(photo=None))
    )
