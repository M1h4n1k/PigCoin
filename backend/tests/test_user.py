...
def test_user_login(mocker):
    patch_validate_data = mocker.patch(
        "routers.user.validate_tg_data", Mock(return_value=True)
    )
...
async def test_user_login_create(mocker, dummy_user):
    ...
    patch_bot_get_photos = mocker.patch(
        "routers.user.bot.get_user_profile_photos", AsyncMock()
    )
    patch_bot_get_photos.return_value = Mock(total_count=0)
    patch_crud_create_user = mocker.patch(
        "routers.user.crud.users.create_user", Mock(return_value=dummy_user)
    )
...
async def test_user_login_create_referral(mocker, dummy_user):
    ...
    patch_update_user_money = mocker.patch(
        "routers.user.crud.users.update_user_money", Mock()
    )
