...
def test_make_transaction(mocker, dummy_user):
    def patch_get_user():
        return Mock(tg_id=1, picture='test', username='test', total_coins=500, current_coins=500)
    get_user_patch = mocker.patch('routers.user.crud.users.get_user', Mock(return_value=dummy_user))
    ...
    client_l = get_client_with_dep(patch_get_user)
    response = client_l.post('/api/user/transactions', json={'to_user_id': dummy_user.tg_id, 'amount': 100})
    assert response.status_code == 200
    assert get_user_patch.mock_calls == [call(ANY, dummy_user.tg_id)]
...
def test_make_transaction_no_coins(mocker, dummy_user):
    def patch_get_user():
        return Mock(tg_id=1, picture='test', username='test', total_coins=500, current_coins=500)
    mocker.patch('routers.user.crud.users.get_user', Mock(return_value=dummy_user))
    make_transaction_patch = mocker.patch('routers.user.crud.users.make_transaction')
    client_l = get_client_with_dep(patch_get_user)
    response = client_l.post('/api/user/transactions', json={'to_user_id': dummy_user.tg_id, 'amount': 1000})
...
def test_make_transaction_to_self(mocker):
    def patch_get_user():
        return Mock(uid=1, picture='test', username='test', total_coins=500, current_coins=500)
    mocker.patch('routers.user.crud.users.get_user', Mock(return_value=None))
    make_transaction_patch = mocker.patch('routers.user.crud.users.make_transaction')
    client_l = get_client_with_dep(patch_get_user)
    response = client_l.post('/api/user/transactions', json={'to_user_uid': 1, 'amount': 100})
    assert response.status_code == 400
    assert make_transaction_patch.mock_calls == []
