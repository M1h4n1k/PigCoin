...
def test_buy_boost(mocker, dummy_user):
    ...
    patch_crud_user = mocker.patch(
        "routers.boosts.crud.users.update_user_money", Mock(return_value=dummy_user)
    )
    patch_crud_boost_buy = mocker.patch(
        "routers.boosts.crud.boosts.buy_boost", Mock(return_value=None)
    )
...
def test_use_turbo(mocker, dummy_user):
    ...
    client_l = get_client_with_dep(patch_get_user)
    patch_crud_user = mocker.patch(
        "routers.boosts.crud.users.user_use_free_turbo", Mock(return_value=dummy_user)
    )
...
def test_use_refill(mocker, dummy_user):
    ...
    client_l = get_client_with_dep(patch_get_user)
    patch_crud_user = mocker.patch(
        "routers.boosts.crud.users.user_use_free_refill", Mock(return_value=dummy_user)
    )
