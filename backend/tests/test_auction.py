...
from .utils import get_client_with_dep, client
...
def test_bet(mocker, dummy_user):
    ...
    get_decoration_patch = mocker.patch(
        "routers.user.crud.decorations.get_decoration_by_id",
        Mock(return_value=decoration_mock),
    )
    make_decoration_patch = mocker.patch(
        "routers.user.crud.decorations.make_bet", Mock(return_value=decoration_mock)
    )

    client_l = get_client_with_dep(patch_get_user_auc)
    response = client_l.post(
        "/api/decorations/bet",
        json={"decoration_id": decoration_mock.id, "amount": 10000},
    )
...
def test_bet_not_enough_coins(mocker, dummy_user):
    ...
    response = client_l.post(
        "/api/decorations/bet",
        json={"decoration_id": decoration_mock.id, "amount": 100000},
    )
...
def test_get_auction(mocker, dummy_user):
    decoration_mock = deepcopy(decoration_mock_)
    get_decoration_patch = mocker.patch(
        "routers.user.crud.decorations.get_decoration_on_auction",
        Mock(return_value=decoration_mock),
    )
