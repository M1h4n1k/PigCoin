...
async def setup_spam_message(message: Message, state: FSMContext):
    ...
    await state.set_data(
        {"spam_text": message.text.replace("/spam ", ""), "spam_entities": entities}
    )
...
async def send_spam_message(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer(
        f"Starting spam to all users, {len(user_ids)} users in total"
    )
    for user in user_ids:
        try:
            ...
            pass
    await call.message.answer(
        f"Done, the message was received by {success_count} users"
    )
