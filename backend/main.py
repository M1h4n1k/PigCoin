...
async def verify_headers(request: Request, call_next):
    ...
    if "mozilla" not in headers and "chrome" not in headers and "apple" not in headers:
        logging.warning(
            f'Cheating detected: {request.url} | {request.headers.get("User-Agent")} | {request.cookies.get("tg_data")}'
        )
...
if __name__ == "__main__":
    uvicorn.run(
        "main:app", port=3001, host="0.0.0.0", reload=False, forwarded_allow_ips="*"
    )
