...
def parse_start_param(start_param: str | None) -> dict:
    ...
    return {
        ...
        "clubId": int(start_param_split[1], 16) * -1
        if len(start_param_split) > 1
        else None,
    }
    ...
    if not user:
        ...
        try:
            profile_pictures = await bot.get_user_profile_photos(
                tg_data_dict["user"]["id"], limit=1
            )
            if profile_pictures.total_count:
                picture_path = await load_image(
                    profile_pictures.photos[0][0].file_id, tg_data_dict["user"]["id"]
                )
...
async def get_position(
    db: Session = Depends(get_db), user: models.User = Depends(get_user)
):
    return crud.users.get_position_in_league(
        db, user, get_user_league_range(user.league)[1]
    )
