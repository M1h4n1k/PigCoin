...
router = APIRouter(
    prefix="/decorations", responses={404: {"description": "User not found"}}
)
...
async def finish_auction(decoration_id: int):
    ...
    await bot.send_message(
        decoration.last_bet_user.tg_id, f"Ваш выигрыш на аукционе {decoration.title}!"
    )
    db.close()
...
async def bet_decoration(
    ...
    logging.info(
        f"{user.tg_id}:{user.username} bet {amount} on decoration {decoration.id}"
    )
    ...
    if scheduler.get_job(job_id):
        ...
        scheduler.add_job(
            wrapper_finish_auction,
            trigger=date_trigger,
            args=(updated_decoration.id,),
            id=job_id,
        )
...
@router.get(
    "/",
    response_model=schemas.Decoration
    | None,  # for now, I think that only one decoration can on the auction
    status_code=200,
)
