...
async def complete_task(
    task_id: int, db: Session = Depends(get_db), user: models.User = Depends(get_user)
):
    ...
    if task.type == "subscribe":
        user_channel_status = await bot.get_chat_member(
            chat_id="@" + task.link.split("/")[-1], user_id=user.tg_id
        )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://music.evicon.fun/api/is_user?user_id={user.tg_id}&key=1AaBMuMKda2Jcbrt"
            ) as resp:
...
async def complete_ad_task(
    ...
    if not user.can_collect_ad:
        raise HTTPException(
            status_code=425, detail="You can watch ads only once in 1.2 hours"
        )
