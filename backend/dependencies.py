...
def validate_tg_data(tg_data: str) -> bool:
    secret_key = hmac.new(
        b"WebAppData", msg=TOKEN.replace("/test", "").encode(), digestmod=sha256
    ).digest()
    if not tg_data:
        return False
    ...
    payload = "\n".join(
        [f"{key}={value}" for key, value in sorted_items if key != "hash"]
    )
    hashed_data = hmac.new(
        secret_key, msg=payload.encode(), digestmod=sha256
    ).hexdigest()
...
async def get_user(
    tg_data: dict = Depends(get_tg_data), db=Depends(get_db)
) -> models.User:
